...
func (h *TaskHandler) CreateTask(ctx context.Context, arg *connect.Request[task_v1.CreateTaskRequest]) (*connect.Response[task_v1.CreateTaskResponse], error) {
	...
	createdID, err := h.ITaskUsecase.CreateTask(ctx, dto.NewCreateTaskParams(
		uid,
		arg.Msg.GetTitle(),
		descriptionPtr,
		arg.Msg.GetStatus(),
...
func (h *TaskHandler) UpdateTaskDetails(ctx context.Context, arg *connect.Request[task_v1.UpdateTaskDetailsRequest]) (*connect.Response[task_v1.UpdateTaskDetailsResponse], error) {
	...
	if err := h.ITaskUsecase.UpdateTaskDetails(ctx, dto.NewUpdateTaskDetailsParams(
		arg.Msg.GetTaskId(),
		uid,
		arg.Msg.GetTitle(),
		descriptionPtr,
...
func (h *TaskHandler) UpdateTaskDueDate(ctx context.Context, arg *connect.Request[task_v1.UpdateTaskDueDateRequest]) (*connect.Response[task_v1.UpdateTaskDueDateResponse], error) {
	// var uid string
	// var err error
	// if uid, err = h.IContextReader.GetUserID(ctx); err != nil {
	// 	return nil, connect.NewError(connect.CodeUnauthenticated, err)
	...
	params := dto.NewUpdateTaskDueDateParams(
		arg.Msg.GetTaskId(),
		dueDateStr,
		dueDate,
	)

	if err := h.ITaskUsecase.UpdateTaskDueDate(ctx, params); err != nil {
		switch e := err.(type) {
		case *application.ErrInputValidationFailed:
			return nil, connect.NewError(connect.CodeInvalidArgument, e)
		case *domain.ErrValidationFailed:
	...
	return connect.NewResponse(&task_v1.UpdateTaskDueDateResponse{}), nil
}

func (h *TaskHandler) UpdateTaskStatus(ctx context.Context, arg *connect.Request[task_v1.UpdateTaskStatusRequest]) (*connect.Response[task_v1.UpdateTaskStatusResponse], error) {
	// var uid string
	// var err error
	// if uid, err = h.IContextReader.GetUserID(ctx); err != nil {
	// 	return nil, connect.NewError(connect.CodeUnauthenticated, err)
	...
	entityStatus := convert.ConvertTaskStatusToEntityStatus(arg.Msg.GetStatus())

	statusString := convert.ConvertEntityStatusToString(entityStatus)

	params := dto.NewUpdateTaskStatusParams(
		arg.Msg.GetTaskId(),
		statusString,
		statusString,
	)

	if err := h.ITaskUsecase.UpdateTaskStatus(ctx, params); err != nil {
		switch e := err.(type) {
		case *application.ErrInputValidationFailed:
			return nil, connect.NewError(connect.CodeInvalidArgument, e)
		case *domain.ErrValidationFailed:
	...
	return connect.NewResponse(&task_v1.UpdateTaskStatusResponse{}), nil
}
